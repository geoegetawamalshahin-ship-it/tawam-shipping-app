...
class _RequestQuoteScreenState extends State<RequestQuoteScreen> {
  late final RequestQuoteFormController _c;
  final _formKey = GlobalKey<FormState>();

  TextEditingController get _pickupController => _c.pickupController;
  TextEditingController get _deliveryController => _c.deliveryController;
  ...
  TextEditingController get _dateController => _c.dateController;
  TextEditingController get _nameController => _c.nameController;
  TextEditingController get _phoneController => _c.phoneController;
  TextEditingController get _emailController => _c.emailController;
  TextEditingController get _notesController => _c.notesController;

  List<String> get _cargoTypes => _c.cargoTypes;
  List<String> get _shippingModes => _c.shippingModes;

  String get _selectedCargoType => _c.selectedCargoType.value;
  set _selectedCargoType(String value) => _c.selectedCargoType.value = value;
  String get _selectedShippingMode => _c.selectedShippingMode.value;
  set _selectedShippingMode(String value) =>
      _c.selectedShippingMode.value = value;
  ...
  void initState() {
    super.initState();
    _c = Get.find<RequestQuoteFormController>();
  }
  ...
  Future<void> _choosePickupDate() async {
    ...
    _c.applyPickupDate(selectedDate, _formatDate(selectedDate));
  }
  ...
  Future<void> _submitQuote() async {
    ...
    if (outcome.isSuccess) {
      // Capture the route before the controller clears the fields, so the
      // success dialog can still show where the shipment was going.
      final routeText =
          '${_pickupController.text} → ${_deliveryController.text}';
      _c.clearForm();
      _showSuccessDialog(routeText);
      return;
    }
    ...
    switch (outcome.error) {
      ...
      default:
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(l10n.couldNotSubmitQuoteRetry)));
        return;
    }
  }

  void _showSuccessDialog(String routeText) {
    final l10n = AppLocalizations.of(context)!;
    showDialog(
      context: context,
                        ...
                        children: _shippingModes.map((mode) {
                          return SizedBox(
                            ...
                            child: _ShippingModeCard(
                              ...
                              onTap: () => _selectedShippingMode = mode,
                            ),
                          );
                        }).toList(),
                      ),
