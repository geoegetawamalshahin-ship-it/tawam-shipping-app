...
| `ShippingNotesField` | `notes_field.dart` | Car, land, parcel, international, sea, **air**. Air `showPrefixIcon: false`, `hintHeight: null`, `includeIdleBorderSide: false`. |
...
| `ShippingPremiumCard` | `premium_card.dart` | Six forms (default radius **21**, blur **18**, offset `(0, 7)`). Get-quote `_card` radius **20** blur **12** offset `(0, 5)`. Booking `_premiumCard` radius 21 blur **14** offset `(0, 5)`. Calculator dimensions card radius **22** blur 14 offset `(0, 5)`. Date rows, calculator empty/results/note stay on pages. |
...
| `ShipmentInfoCard` | `lib/app/widgets/shipment_status/info_card.dart` | Track `_InformationCard` `minHeight` **106**; details `_InfoCard` **107**. Colors stay on adapters. Details `_WideInfoCard` stays separate. |
| `ShipmentSquareButton` / `ShipmentLiveDot` / `ShipmentHeroFeature` | `lib/app/widgets/shipment_status/` | Track and support keep icon **13** / text **9**. Booking `_HeroFeature` icon **14** / text **9.5**. Calculator `_HeroFeature` icon **14** / text **9.3**. `_TrustFeature` stays booking-only. |
...
| Details `_InfoCard` | — | Extracted as `ShipmentInfoCard` with `minHeight` 106 (track) vs 107 (details). `_WideInfoCard` stays details-only. |
...
Air notes uses `ShippingNotesField` with `showPrefixIcon: false`, `hintHeight: null`, `includeIdleBorderSide: false`; international special-item chips; air unstyled Done and sea `letterSpacing: .35` **via dialog parameters**; six-form quote success vs action success dialogs (different buttons/layout); booking/get-quote **date rows** (full card vs height-62 field); booking profile load (`TAWAM Customer`, extra keys, phone controller); support heading vs numbered heading; field decorations across support/quote/search (not auth); list date fallbacks (`-` / `notProvided` / `notSpecified`); request-quote quantity vs land/sea; booking SnackBar (`0xFF9D2732`); filled shipping snackbars vs unfilled track/support snackbars; documents viewer and documents empty state; notifications list, notifications empty, and notifications filter chips; shipments empty (tile + filter button); Home vs Profile headers/logout; details `_WideInfoCard`; login vs forgot vs quote/support **email validators**; calculator empty results / gradient result / yellow note.
...
**Report, not reverted:** `shipment_status_widgets.dart` became a barrel exporting `square_button.dart`, `live_dot.dart`, `hero_feature.dart`, `timeline_row.dart`, `info_card.dart`. Same widgets, file split for organization. Call sites still use them.
...
- Notes + customer details on five forms; air now also calls `ShippingNotesField` (`showPrefixIcon: false`, `hintHeight: null`, `includeIdleBorderSide: false`) and `ShippingCustomerDetails` with `verifiedMessageHeight: null` and `labelFontWeight: null` so air type stays as before.
...
- `ShippingPremiumCard` also wraps get-quote, booking, and calculator dimension shells (radius/blur/offset stay on adapters). Date rows stay on pages.
- Track/details compact info cards share `ShipmentInfoCard` (`minHeight` 106 vs 107). Details `_WideInfoCard` stays separate.
- Hero row chips share `ShipmentHeroFeature` (track/support 13/9; booking 14/9.5; calculator 14/9.3). Booking `_TrustFeature` stays separate.
